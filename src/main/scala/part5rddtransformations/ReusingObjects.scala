...
import org.apache.hadoop.shaded.org.xbill.DNS.Serial
...
object ReusingObjects {
  ...
  def generateData(): Unit = {
    DataGenerator.generateText("src/main/resources/generated/lipsum/3m.txt", 6000000, 3000000, 200)
  }

  // Version 1 - Immutable Objects
  ...
  def collectStats() = {
    ...
    def aggregateNewRecord(textStats: TextStats, record: String): TextStats = {
      ...
      TextStats(
        ...
        if (longestWord.length > textStats.maxWordLength) longestWord.length
        else textStats.maxWordLength,
    ...
    def combineStats(stats1: TextStats, stats2: TextStats): TextStats = {
      TextStats(
        ...
        if (stats1.maxWordLength > stats2.maxWordLength) stats1.maxWordLength
        else stats2.maxWordLength,
        stats1.occurrences + stats2.occurrences
      )
    }

    val aggregate: RDD[(String, TextStats)] =
      text.aggregateByKey(TextStats.zero)(aggregateNewRecord, combineStats)
    aggregate.collectAsMap()
  }

  // Version 2 - Mutable Objects

  class MutableTextStats(
      var nLines: Int,
      var nWords: Int,
  ...
  def collectStats2() = {
    ...
    val aggregate: RDD[(String, MutableTextStats)] =
      text.aggregateByKey(MutableTextStats.zero)(aggregateNewRecord, combineStats)
    aggregate.collectAsMap()
  }
  ...
  object uglyTextStats extends Serializable {
    ...
    // maxBy() convert arrays into scala Seq() collections which is not efficient
    def aggregateNewRecord(textStats: Array[Int], record: String): Array[Int] = {
      ...
      while (i < newWords.length) {
        ...
        textStats(longestWordIndex) =
          if (wordLength > textStats(longestWordIndex)) wordLength else textStats(longestWordIndex)
    ...
    def combineStats(stats1: Array[Int], stats2: Array[Int]): Array[Int] = {
      ...
      stats1(longestWordIndex) =
        if (stats1(longestWordIndex) > stats2(longestWordIndex)) stats1(longestWordIndex)
        else stats2(longestWordIndex)
  ...
  def collectStats3() = {
    ...
    val aggregate: RDD[(String, Array[Int])] = text.aggregateByKey(Array.fill(4)(0))(
      uglyTextStats.aggregateNewRecord,
      uglyTextStats.combineStats
    )
    aggregate.collectAsMap()
  }
