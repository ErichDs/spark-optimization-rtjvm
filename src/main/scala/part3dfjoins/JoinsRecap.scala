...
object JoinsRecap {
  ...
  // (full) outer join = everything in left_outer + rigth_outer
  ...
  // cross join = combines everything in the left table with everything in the right table
  ...
  val scoresRDD: RDD[(String, (Int, Int))] = wordsRDD.join(colorsRDD) // implied join type is INNER
