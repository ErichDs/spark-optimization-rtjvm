...
object BroadcastJoins {
  ...
  // large table
  val table = spark.range(1, 100000000) // wher you use spark.range, the column is "id"
...
//  joinedSmart.show()
  ...
  val smallerTable = spark.range(1, 10000) // size estimated by Spark - auto-broadcast
  val joinedNumbers = smallerTable.join(bigTable, "id")

  // deactivate auto-broadcast
  spark.conf.set("spark.sql.autoBroadcastJoinThreshold", -1) // default: 10485760 (10 mb)
