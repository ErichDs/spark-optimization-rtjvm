...
object RDDBroadcastJoins {
  ...
  val improvedMedalists = leaderboard.mapPartitions {
    iterator => // iterator of all the tuples in this partition; all the tuples are local to this executor
      iterator.flatMap { record =>
        val (index, name) = record
        medalsMap
          .get(index) match { // notice you can refer to the name medalsMap, which you now have access to locally after the broadcast
          case None        => Seq.empty
