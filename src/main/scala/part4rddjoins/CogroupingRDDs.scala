...
object CogroupingRDDs {
  ...
  /*
   Take all the student attemps
    - if a student passed(at least one attempt > 9.0), send them an email "PASSED"
    - else send them an email "FAILED"
   */

  def readIds() = sc.textFile(s"$rootFolder/examIds.txt").map { line =>
    val tokens = line.split(" ")
    (tokens(0).toLong, tokens(1))
  }

  def readExamScores() = sc.textFile(s"$rootFolder/examScores.txt").map { line =>
    val tokens = line.split(" ")
    (tokens(0).toLong, tokens(1).toDouble)
  }

  def readExamEmails() = sc.textFile(s"$rootFolder/examEmails.txt").map { line =>
    val tokens = line.split(" ")
    (tokens(0).toLong, tokens(1))
  }
  ...
  def plainJoin() = {
    ...
    val results = candidates
      ...
      .mapValues { case ((name, maxAttempt), email) =>
  ...
  def coGroupedJoin() = {
    val scores =
      readExamScores().reduceByKey(Math.max) // make sure we're getting only the max result
    val candidates = readIds()
    val emails     = readExamEmails()
    ...
    val result: RDD[(Long, Option[(String, String)])] =
      candidates
        .cogroup(
          scores,
          emails
        ) // co-partition the 3 RDDs: RDD[(Long, (Iterable[String], Iterable[Double], Iterable[String]))]
        .mapValues { case (nameIterable, maxAttemptIterable, emailIterable) =>
